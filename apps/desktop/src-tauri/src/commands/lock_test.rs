...
fn second_instance_cannot_acquire_the_same_workspace() {
    ...
    let workspace =
        std::env::temp_dir().join(format!("openbible-lock-test-{}", std::process::id()));
    ...
    assert_eq!(
        second.expect_err("second lock must fail").code(),
        "workspace_locked"
    );
...
fn initialize_is_idempotent_for_the_active_workspace() {
    // SPECSFY: US-003 FR-004 NFR-001 AC-009
    let workspace =
        std::env::temp_dir().join(format!("openbible-initialize-test-{}", std::process::id()));
    let mut context = crate::commands::workspace::WorkspaceContext::default();
    let first = crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    )
    .expect("first initialization");
    let second = crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    )
...
fn missing_workspace_file_is_an_empty_read() {
    ...
    let mut context = crate::commands::workspace::WorkspaceContext::default();
    crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    )
    ...
    let result =
        crate::commands::workspace::read_file(&context, ".openbible/preferences.json".into())
...
fn delete_file_removes_a_workspace_relative_file() {
    ...
    let workspace =
        std::env::temp_dir().join(format!("openbible-delete-test-{}", std::process::id()));
    let mut context = crate::commands::workspace::WorkspaceContext::default();
    crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    ...
    crate::commands::workspace::delete_file(&context, "notes/example.md".into())
        .expect("delete note");
...
fn inspect_bible_accepts_openlp_schema_without_abbreviation() {
    // SPECSFY: US-001 FR-002 NFR-003 AC-004
    let workspace =
        std::env::temp_dir().join(format!("openbible-bible-test-{}", std::process::id()));
    let mut context = crate::commands::workspace::WorkspaceContext::default();
    crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    ...
    let info = crate::commands::workspace::inspect_bible_impl(&context, "bibles_ACF.sqlite".into())
        .expect("inspect bible");
...
fn inspect_bible_accepts_null_book_abbreviation() {
    // SPECSFY: US-001 FR-002 NFR-003 AC-004
    let workspace =
        std::env::temp_dir().join(format!("openbible-null-abbr-test-{}", std::process::id()));
    let mut context = crate::commands::workspace::WorkspaceContext::default();
    crate::commands::workspace::initialize(
        &mut context,
        Some(workspace.to_string_lossy().into_owned()),
    ...
    let info = crate::commands::workspace::inspect_bible_impl(&context, "bibles_ARA.sqlite".into())
        .expect("inspect bible");
    assert_eq!(info.books[0].abbreviation, "");
